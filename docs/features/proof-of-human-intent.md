 ...
 **Customize which actions require PoHI verification** — with pre-configured exceptions like high-value transactions and account recovery, for which strong verification is always required.
 
 **PoHI triggers automatically if a user unknowingly initiated a critical action** (blind signing, phishing...), **alerting them to the true intent behind the action**.
